...
def FUNCTION_GRAPH_1(datos_TIME):
    ...
    # Llamar a create_bar_chart con df1 y df3
    components_bar_chart = create_bar_chart(df1, df3)
    components_pie_chart = create_pie_chart(df2, df4)

    return components_bar_chart, components_pie_chart

def create_bar_chart(df1, df3):
    fig_1 = make_subplots(rows=1, cols=2, subplot_titles=("Horas de Formación Directa [FD]", "Horas de Formación Autónoma [FA]"))
    ...
    return pio.to_html(fig_1, full_html=False)

def create_pie_chart(df2, df4):
    fig_2 = make_subplots(rows=2, cols=2, specs=[[{'type':'pie'}, {'type':'pie'}]], subplot_titles=("Porcentaje de formación directa [FD]", "Porcentaje de formación Autónoma [FA]"))

    fig_2.add_trace(
        go.Pie(labels=df2["Competencia"], values=df2["Horas de formación directa [FD]"]),
        2, 1
    )

    fig_2.add_trace(
        go.Pie(labels=df4["Competencia"], values=df4["Horas de formación autónoma [FA]"]),
        2, 2
    )
